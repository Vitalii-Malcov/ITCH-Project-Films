...
class MockProvider(AIImageProvider):
    ...
    def generate(
        ...
        try:
            ...
            # seed принимается для совместимости с интерфейсом
            # AIImageProvider (см. base.py), но MockProvider не
            # использует случайность — цвет полностью определяется
            # текстом промпта (_color_from_prompt), поэтому результат
