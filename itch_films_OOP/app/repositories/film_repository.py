...
#
# ОТЛИЧИЕ ОТ ПРОЦЕДУРНОЙ ВЕРСИИ (mysql_connector.py):
#   - было:  отдельные функции модуля + @lru_cache(maxsize=1)
#   - стало: методы класса + кэш хранится в атрибутах self
#            (self._genres_cache / self._year_range_cache).
#            Смысл тот же (посчитать один раз за время жизни
#            процесса), но состояние явно принадлежит объекту,
#            а не "болтается" на уровне модуля.
# ─────────────────────────────────────────────
